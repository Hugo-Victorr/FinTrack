using FinTrack.Database.EFDao;
using FinTrack.Database.Repository;
using FinTrack.Model.Entities;

namespace FinTrack.Database.Interfaces;

public class ExpenseCategoryRepository : BaseDao<ExpenseCategory>, IExpenseCategoryRepository
{
    public ExpenseCategoryRepository(FintrackDbContext context) : base(context)
    {
    }

    protected override Task ValidateEntityForInsert(params ExpenseCategory[] obj)
    {
        return Task.CompletedTask;
    }

    protected override Task ValidateEntityForUpdate(params ExpenseCategory[] obj)
    {
        return Task.CompletedTask;
    }
}





